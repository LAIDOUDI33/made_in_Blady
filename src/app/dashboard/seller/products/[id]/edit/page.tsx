...
// Form data types (same as New Product)
...
const mockProductData: ProductFormData = {
  ...
  sku: 'CPT-CEM42-001',
  shortDescription: 'Ciment Portland de haute qualité pour travaux de construction et génie civil. Conforme aux normes algériennes et internationales.',
  category: 'Matériaux de Construction',
  subcategory: 'Ciment & Liants',
  priceType: 'range',
  price: '',
  priceRangeMin: '5800',
  priceRangeMax: '6200',
  ...
  moq: '100',
  unit: 'sac de 50kg',
  description: `Le Ciment Portland CEM I 42.5 est un ciment d'usage général adapté à tous les ouvrages de construction en béton armé ou non armé.

Caractéristiques principales:
• Classe de résistance 42.5 MPa à 28 jours
• Faible teneur en alcalis
• Résistance aux sulfates améliorée
• Temps de prise adaptable selon les conditions climatiques
...
Conditionnement:
Disponible en sacs de 50kg sur palette de 1 tonne (20 sacs).

Stockage:
Conserver au sec, à l'abri de l'humidité. Durée de conservation: 6 mois après fabrication.`,
  specifications: [
    { key: 'Résistance (28j)', value: '42.5 MPa' },
    { key: 'Densité', value: '3.15 g/cm³' },
    { key: 'Surface spécifique', value: '350 m²/kg' },
    { key: 'Temps de prise initial', value: '≥ 45 min' },
    { key: 'Perte au feu', value: '< 5%' },
    { key: 'Résidu (90µm)', value: '< 10%' },
  ],
  leadTime: '2-5 jours ouvrables',
  countryOfOrigin: 'Algérie',
  images: [
    { id: '1', url: '/uploads/cement-1.jpg', isPrimary: true },
    { id: '2', url: '/uploads/cement-2.jpg', isPrimary: false },
  ],
};
...
export default function EditProductPage() {
  ...
  const handleSubmit = async (action: 'draft' | 'publish') => {
    ...
    try {
      ...
      alert(action === 'draft' ? 'Produit sauvegardé !' : 'Produit mis à jour avec succès !');
                      ...
                      {formData.images.length > 0 && (
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {formData.images.map((image) => (
                            ...
                            >
                              <img src={image.url} alt="Product" className="w-full h-40 object-cover" />
                      ...
                      <div className="bg-white p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center">
                            {formData.images.find((img) => img.isPrimary) ? (
                              ...
                              <img src={formData.images[0].url} alt={formData.name} className="w-full h-full object-cover" />
                            ) : (
                          ...
                          <div className="space-y-4">
                            ...
                            {formData.shortDescription && (
                              <p className="text-gray-600 text-sm line-clamp-3">{formData.shortDescription}</p>
                            )}
                        ...
                        {formData.description && (
                          <div className="mt-8 pt-6 border-t">
                            ...
                            <p className="text-gray-600 whitespace-pre-wrap text-sm">{formData.description}</p>
                          </div>
                        )}
                        ...
                        {formData.specifications.some((s) => s.key && s.value) && (
                          <div className="mt-6 pt-6 border-t">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {formData.specifications.filter((s) => s.key && s.value).map((spec, i) => (
