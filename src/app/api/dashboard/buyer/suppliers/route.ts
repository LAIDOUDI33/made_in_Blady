...
export async function GET(request: NextRequest) {
  try {
    ...
    const hasNewProducts = searchParams.get('hasNewProducts'); // Filter suppliers with new products
    ...
    const [followedSuppliers, total] = await Promise.all([
      db.favorite.findMany({
        where,
        include: {
          // Since we're storing company ID in itemId for SUPPLIER type,
          // we need to fetch company details separately or use a relation
        },
    ...
    const result = followedSuppliers.map(follow => {
      ...
      return {
        ...
        ...(company ? {
          ...
          slug: company.slug,
          logo: company.logo,
          category: company.description?.substring(0, 50), // Would be better to have a dedicated field
          ...
          hasNewProducts: false, // Would need product creation date tracking
          lastActive: company.updatedAt.toISOString(),
          description: company.description
        } : null)
      };
    }).filter(r => r.name); // Remove if company not found
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { supplierId, action } = body; // action: follow, unfollow
    ...
    switch (action) {
      ...
      case 'toggle_notifications':
        ...
        // In a real app, you'd have a SupplierNotification preference table
