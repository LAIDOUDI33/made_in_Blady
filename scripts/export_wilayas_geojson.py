#!/usr/bin/env python3
"""
Generate GeoJSON Export for Algeria Wilayas
============================================
Creates GeoJSON format suitable for GIS applications (Leaflet, Mapbox, QGIS).
Includes point geometries for each wilaya capital.
"""
...
def create_geojson():
    """Create GeoJSON FeatureCollection with all wilayas as points."""
    
    features = []
    
    for w in ALL_WILAYAS:
        # Create a point geometry for each wilaya capital
        feature = {
            "type": "Feature",
            "geometry": {
                "type": "Point",
                "coordinates": [
                    w['coordinates']['lng'],  # Longitude first for GeoJSON
                    w['coordinates']['lat']   # Latitude second
                ]
            },
            "properties": {
                "code": w['code'],
                ...
                "communes": w['communes'],
                "region": w['region'],
                "region_group": get_region_group(w['region']),
    ...
    geojson = {
        ...
        "features": features,
        "metadata": {
            "title": "Algeria's 58 Wilayas - Capital Cities",
            "description": "GeoJSON point dataset of Algerian provincial capitals with administrative data",
            ...
            "bounds": {
                "min_lng": min(w['coordinates']['lng'] for w in ALL_WILAYAS),
                "min_lat": min(w['coordinates']['lat'] for w in ALL_WILAYAS),
                "max_lng": max(w['coordinates']['lng'] for w in ALL_WILAYAS),
                "max_lat": max(w['coordinates']['lat'] for w in ALL_WILAYAS)
            },
            "usage_notes": [
                "Each point represents a wilaya capital city",
                "Coordinates are in WGS84 (EPSG:4326)",
                "Suitable for Leaflet, Mapbox, QGIS, ArcGIS",
                "Population figures from latest available census"
            ]
        }
    }
...
def main():
    ...
    output_path = '/home/z/my-project/download/algeria_58_wilayas_capitals.geojson'
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(geojson, f, ensure_ascii=False, indent=2)
    
    print(f"✅ GeoJSON export saved to: {output_path}")
    print(f"\n📍 Geographic Bounds:")
    bounds = geojson['metadata']['bounds']
    print(f"   West: {bounds['min_lng']}°E")
    print(f"   East: {bounds['max_lng']}°E")
    print(f"   South: {bounds['min_lat']}°N")
    print(f"   North: {bounds['max_lat']}°N")
    print(f"\n📊 Total Features: {geojson['metadata']['total_features']}")
    print("\n💡 Compatible with:")
    print("   • Leaflet.js / Mapbox GL")
    print("   • QGIS / ArcGIS Pro")
    print("   • Google Earth Pro")
    print("   • Any GIS application supporting GeoJSON")
