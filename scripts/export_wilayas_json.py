#!/usr/bin/env python3
"""
Export Algeria Wilayas Data to JSON format
============================================
Creates a clean JSON file with all 58 wilayas for API/data use.
"""
...
def main():
    """Generate JSON export of all wilayas data."""
    ...
    output = {
        "metadata": {
            "title": "Algeria's 58 Wilayas - Complete Dataset",
            "description": "Comprehensive administrative data for all Algerian provinces",
            "generated_at": datetime.now().isoformat(),
            "total_wilayas": len(ALL_WILAYAS),
            "total_population": total_population,
            "total_area_km2": total_area,
            "data_sources": [
                "Wikipedia (EN/FR/AR)",
                "Official Algerian Government Sources",
                "ONS Algeria Statistics"
            ]
        },
        "wilayas": ALL_WILAYAS,
        "summary": {
            "most_populous": max(ALL_WILAYAS, key=lambda x: x['population'])['name_french'],
            "least_populous": min(ALL_WILAYAS, key=lambda x: x['population'])['name_french'],
            "largest_area": max(ALL_WILAYAS, key=lambda x: x['area_km2'])['name_french'],
            "smallest_area": min(ALL_WILAYAS, key=lambda x: x['area_km2'])['name_french'],
            "average_population": total_population // len(ALL_WILAYAS),
            "average_area": total_area // len(ALL_WILAYAS)
        }
    }
    ...
    # Write JSON file
    output_path = '/home/z/my-project/download/algeria_58_wilayas_data.json'
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    
    print(f"✅ JSON export saved to: {output_path}")
    print(f"\n📊 Dataset Summary:")
    print(f"   - Total Wilayas: {len(ALL_WILAYAS)}")
