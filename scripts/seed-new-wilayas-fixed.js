/**
 * Fixed script to seed NEW wilayas 51-58 with all required fields
 */
...
async function main() {
  console.log('🚀 Seeding NEW Wilayas 51-58 (Fixed Version)...\n');
  ...
  for (const wilaya of NEW_WILAYAS) {
    ...
    try {
      ...
      totalAdded += added;
      const icon = added > 0 ? '✅' : (errors > 0 ? '⚠️' : 'ℹ️ ');
      console.log(`${icon} [${wilaya.code}] ${wilaya.name}: +${added} (${errors} errors)`);
